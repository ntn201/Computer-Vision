...
img = cv2.imread("Image\contour2.png")
...
for i in contours:
    ...
    epsilon = 0.1*cv2.acrLength(i,True)
    nedges = cv2.approxPolyDP(i,epsilon,True)
    ...
    if len(nedges) == 4:
        x = nedges[0][0][0]
        y = nedges[0][0][1]
        cv2.putText(img,"Rectangle",(x,y-20),cv2.FONT_HERSHEY_COMPLEX,0.5,(0,255,0))
    if len(nedges) >= 10:
        x = nedges[0][0][0]
        y = nedges[0][0][1]
        cv2.putText(img,"Circle",(x-30,y-10),cv2.FONT_HERSHEY_COMPLEX,0.5,(0,255,0))
